...
def get_triangular_opportunity(tickers: List['ShortTicker']) -> Tuple[List['ShortTicker'], float]:
    ...
    for ticker in tickers:
        if ticker.symbol is not None:
            ...
            reversed_ticker = ShortTicker(Symbol(ticker.symbol.quote, ticker.symbol.base),
    ...
    if best_triplet is not None:
        best_triplet = [
            ShortTicker(Symbol(triplet.symbol.quote, triplet.symbol.base), triplet.last_price,
                        reversed=True)
